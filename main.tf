  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "AP_DATA_PLANE_ZONE_SLUG", value = var.self_hosted_data_plane ? "test" : "" },
        { name = "AP_DATA_PLANE_INTERNAL_HOST", value = var.self_hosted_data_plane ? "http://${aws_alb.internal[0].dns_name}" : "" },
        { name = "AP_DATA_PLANE_EXTERNAL_URL", value = var.self_hosted_data_plane ? "http://${aws_alb.external[0].dns_name}" : "" },
